...
class GPDR(
    ...
    // R_0, R_1, ... are a sequence of over-approximations of the reachable states (in i or fewer steps)

    // Invariants:
    // R_i under-approximates S: {R_i} \subseteq {S}
    // R_{i+1} over-approximates R_i and predicateTransform(R_i): {R_i} \subseteq {R_{i+1}}, {predicateTransform(R_i)} \subseteq {R_{i+1}}

    fun check(): Boolean {
        // INITIALIZE
        val candidateModels: MutableList<Pair<BooleanExpression, Int>> = mutableListOf()
        val approximations: MutableList<BooleanExpression> = mutableListOf(transitionSystem.initial) // initial == F(false)
        var N = 0
        ...
        while (i < bound) {
            ...
            // INDUCTION: We may already have inductive properties in our R_i (just not strong enough for our final goal)
            ...
            if (candidateModels.isEmpty()) {
                val test = Entailment(approximations[N], Not(transitionSystem.invariant), "Searching for candidate model")
    ...
    private fun Map<String, String>.toFormula(): BooleanExpression {
        return this.map { Eq(
            ValAtAddr(Variable(it.key)), NumericLiteral((it.value as Int).toBigInteger()), 0)
    ...
    private fun F(R: BooleanExpression, vars: List<String> = transitionSystem.vars): BooleanExpression {
